...
%%%
%%%
%%%     Los nodos tienen la forma:    S#D#F#A
%%%            Donde S describe el estado o la configuración (state)
%%%                  D is the la profundidad del nodo (depth)
%%%                  F is es el valor de la función de evaluación (function)
%%%                  A is es el antecesor en la lista del nodo (ancestor)

:- op(400,yfx,'#').    /* Notación para construir el nodo*/
...
%%%
%%%
%%%     Los estados tienen la forma:    A/B/C/D/E/F/G/H/I
%%%            donde {A,...,I} = {0,...,8}
%%%               0 representa el cuadro vacío
%%%             
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%
%%%     Animación del 8 Puzzle -- Usando caracteres VT100
%%%
%%%
%%%             
...
animate(P,S) :- initialize(P),
               cursor(1,2), write(S), 
               cursor(1,22), write('Presiona ENTER para resolver paso a paso'),
               get0(_X),
               play_back(S).

:- dynamic location/3.   %%% Con esto, la localización del cuadro
                        %%%  puede ser rechazada o aceptada
...
%%% Atributos de video -- bold and blink not working
plain         :- put(27), put("["), put("0"), put("m").
reverse_video :- put(27), put("["), put("7"), put("m").


%%% Objetos en cuadros, mapa de caracteres
%%% Cada cuadro debería ser impreso de acuerdo al mapa de caracteres,
%%% impreso en 'location', mismo que es aceptado o rechazado por 'PlayBack'.
character_map(N, [ [' ',' ',' ',' ',' ',' ',' '],
                  [' ',' ',' ', N ,' ',' ',' '],
                  [' ',' ',' ',' ',' ',' ',' '] ]).


%%% Mueve el cuadro vacío a la izquierda
